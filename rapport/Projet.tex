...
Dans cette configuration nous traitons de l'installation d'un serveur NFS en version 4 dont le but est d'exporter le contenu un répertoire /tmp vers les clients.
...
Pour vous présenter la méthodologie d'installation des deux parties Serveur et Client, je vais réaliser la plus simple architecture, à savoir deux serveurs qui abriterons le volume distribué et le montage de celle-ci par une machine cliente.\\
...
Actuellement, l'ajout des dépots Sid est obligatoire pour pouvoir accès à cette version de Gluster.\\
...
mkdir sharespace
	  \end{lstlisting}
La génération des fichiers de configuration s'éffecturons à partir de notre première serveur grâce à la commande glusterfs-volgen.\\
J'oubli de vous fournir les ip que nous allons utiliser pour une meilleur compréhension.\\
...
Generating client volfiles.. for transport 'tcp'
	  \end{lstlisting}
Vous pouvez constater que la méthode de transport par défaut est le protocol TCP.\\
...
glusterfs-volgen --name mystore -t ib-verbs 172.16.65.92:/sharespace 172.16.65.91:/sharespace
	  \end{lstlisting}
La commande glusterfs-volgen vous a généré les fichiers de configuration pour les serveurs et le client. Chaque fichier porte le ip de la machine conserné dans son nom, nous allons les exporter dans leur répertoire d'exploitation et les renommer pour les rendre exécutable.
\begin{lstlisting}
...
Si vous définisez la valeur à 0 cela signifira une suppréssion immédiate du dossier et sa récupération ne sera plus possible. \\
Positionner la rétention désiré pour la corbeille mfssettrashtime (option r permet la récurcivité sur les répertoire) et vérifier la rétention appliqué mfsgettrashtime:
\begin{lstlisting}
...
2 copies: 1 chunks
	  \end{lstlisting}
Résumé du contenu de l'arbre entier du file systeme peut être appelé avec la commande mfsdirinfo :
\begin{lstlisting}
...
<<<<<<< HEAD
			\section{Metas\_scripts}
=======
			\section{Meta scripts}
				Fichier meta\_NFS.sh :
				\lstinputlisting[language=Bash]{../metascript/meta_NFS.sh}

>>>>>>> e17902fe3669e31536ce3d2cbb57034b0264c0ea
				Fichier meta\_gluster.sh :
				\lstinputlisting[language=Bash]{../metascript/meta_gluster.sh}

				Fichier meta\_ceph.sh :
				\lstinputlisting[language=Bash]{../metascript/meta_ceph.sh}

				Fichier meta\_moose.sh :
				\lstinputlisting[language=Bash]{../metascript/meta_moose.sh}
\end{document}
