...
\newpage
		        ...
		        \end{figure}
		        \section{Mise en place}
		                       Avant toute configuration, il nous faut déterminier les paquets qui nous seront utiles.            Utilisant une image de Debian Squeeze, On recherche dans la liste des paquets disponibles, ceux dont le nom débute par 'ceph'.
		                       \begin{lstlisting}
...
p   ceph-fuse-dbg          - debugging symbols for ceph-fuse
                           \end{lstlisting}	                       
		                       Afin de mettre en place Ceph, il faut connaitre le nombre exacte de serveur qui seront utilisés. Dans cet exemple de mise en place, nous prenons 2 serveurs, node0 et node1. node0 sera le serveur principal que l'on appel aussi moniteur. La totalité des configurations, hormis le montage des clients, se font à partir du moniteur. Pour commencer, il faut créer un fichier de configuration ceph.conf qui contiendra une grande partie des données nécessaires au bon fonctionnement des serveurs :
		           \begin{lstlisting}
[global]
